...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run stress-test evaluation of learned policies against "
            "handcrafted sanity opponents."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Q-learning stress-test agents can be selected with --agents."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "SARSA stress-test agents can be selected with --agents."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Games per stress-test agent/opponent matchup.",
    )

    parser.add_argument(
        "--agents",
        choices=sorted(SUPPORTED_STRESS_TEST_AGENTS),
        nargs="+",
        default=list(DEFAULT_STRESS_TEST_AGENTS),
        help=(
            "Agents/policies to evaluate against stress-test opponents. "
            "Scripted baselines are also supported for baseline-vs-baseline sanity."
        ),
    )

    parser.add_argument(
        "--opponents",
        choices=sorted(SUPPORTED_STRESS_TEST_OPPONENTS),
        nargs="+",
        default=list(DEFAULT_STRESS_TEST_OPPONENTS),
        help="Handcrafted stress-test opponents.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output CSV path. Default: <training-run-dir>/stress_test_evaluation.csv"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=600_000,
        help="Base seed used to make stress-test evaluation reproducible.",
    )
...
def evaluate_bundle_worker(
    ...
    return evaluate_stress_test_bundle(
        bundle=bundle,
        config=config,
    )
...
def main() -> None:
    ...
    print(
        "Stress-test evaluation started\n"
    ...
    print(
        "Stress-test evaluation finished\n"
        f"bundles={len(bundles)}\n"
        f"rows={len(all_rows)}\n"
