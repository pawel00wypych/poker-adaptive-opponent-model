...
def model_run_name(model_type: str) -> str:
    ...
    raise ValueError(
        f"Unsupported Q-learning model type: {model_type}"
    )


def default_model_path(model_type: str) -> str:
    return str(
        Path("results/models/q_learning")
...
def build_q_learning_metadata(
    *,
    ...
    metadata = {
        "algorithm": Q_LEARNING_ALGORITHM_NAME,
    ...
    *,
    ...
    if model_type == MODEL_TYPE_GENERAL_POLICY:
        return GeneralPolicyPlayer(
            agent=agent,
            player_name="general_policy_q_learning",
    ...
    if model_type in TRAINING_OPPONENT_TYPES:
        return SpecialistPolicyPlayer(
            ...
            player_name="specialist_q_learning",
            verbose=player_verbose,
            log_interval=player_log_interval,
        )
...
def run_q_learning_model_training(
    *,
    model_type: str,
    ...
    for episode_index in range(total_episodes):
        ...
        if (
            ...
            if progress:
                print(
                    "Saved Q-learning checkpoint: "
                    f"{checkpoint_path}"
                )

    training_duration = perf_counter() - training_start

    final_metadata = build_q_learning_metadata(
        model_type=(
            MODEL_TYPE_GENERAL_POLICY
            if model_type == MODEL_TYPE_GENERAL_POLICY
            else MODEL_TYPE_SPECIALIST
