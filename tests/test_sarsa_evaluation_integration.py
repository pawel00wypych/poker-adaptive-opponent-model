...
def make_bundle_with_sarsa_paths(tmp_path: Path) -> ModelBundle:
    return ModelBundle(
...
def test_build_sarsa_unknown_policy_player(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.evaluation.checkpoint_evaluator.load_sarsa_eval_agent",
...
def test_build_sarsa_adaptive_player(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "src.evaluation.checkpoint_evaluator.load_sarsa_adaptive_agents",
    ...
    assert player.player_name == ADAPTIVE_SARSA_AGENT
    assert player.expected_opponent_type == OPPONENT_TYPE_CALLING


def test_sarsa_agents_require_sarsa_run_dir(tmp_path):
    bundle = ModelBundle(
        training_run_directory=tmp_path / "mc_run",
        seed=42,
        checkpoint_episode=2000,
...
def test_build_generalization_sarsa_adaptive_uses_base_family(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setattr(
        "src.evaluation.generalization_evaluator.load_sarsa_adaptive_agents",
        lambda bundle: dummy_agents(),
    )
...
def test_build_generalization_sarsa_unknown_policy(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.evaluation.generalization_evaluator.load_sarsa_eval_agent",
        fake_load_sarsa_eval_agent,
    )
...
def test_experiment_summary_accepts_sarsa_rows(tmp_path):
    rows = []
    for seed in (42, 123):
